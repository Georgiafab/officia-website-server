const { Classfiy, Case, Brand, Counter } = require("../model/index");
...
exports.getCaseList = async (req, res) => {
  try {
    ...
    const str = case_name ? `.*${case_name}.*` : ".";
    const reg = new RegExp(str);
    const searchOpt = {
      $and: [{ case_name: { $regex: reg } }],
    };
    let classfiyItem;
    if (size == 1000) {
      classfiyItem = await Classfiy.findOne({ enTitle: classfiy_id });
      // brand_id = await Brand.findOne({ enTitle: brand_id });
    }
    classfiy_id &&
      (searchOpt["$or"] = [
        { classfiy_id1: classfiy_id },
        { classfiy_id2: classfiy_id },
      ]);
...
exports.getClassfiyList = async (req, res) => {
  try {
    let { page = 1, size = 10, classfiy_name = "", brand_id } = req.query;
    // const searchObj = {};
...
exports.addCounter = async (req, res) => {
  try {
    const counterModel = new Counter(req.body);
